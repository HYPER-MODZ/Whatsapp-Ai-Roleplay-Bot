...
async function performBackup() {
  ...
  try {
    ...
    for (const dir of directories) {
        ...
        // Find or create directory folder
        let dirFolder = backupFolder.children.find(node => node.name === dir.name);
        if (dirFolder) {
            // Delete existing folder contents
            for (const child of dirFolder.children) {
                await deleteItem(child);
            }
        } else {
            // Create new folder
            dirFolder = await createFolder(backupFolder, dir.name);
        }
